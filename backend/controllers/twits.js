...
module.exports.getTwits = (req, res, next) => {
	Twit.find({}).orFail().then((twits) => res.send(twits)).catch((err) => {
		...
		.then((twit) => {
			if (twit.owner.equals(req.user._id)) {
				return Twit.findByIdAndUpdate(
					...
					.orFail()
					.then((updatedTwit) => res.send(updatedTwit))
					.catch((err) => {
						errorHandler(err, next, {
							CastErrorMessage: 'Переданы некорректные данные',
			...
			throw new ForbiddenError(
				'Доступ запрещен. Возможно редактирование только собственного твита'
			);
		})
		.catch((err) => {
			errorHandler(err, next, {
...
module.exports.deleteTwit = (req, res, next) => {
	...
	Twit.findById(twitId)
		...
		.then((twit) => {
			...
			throw new ForbiddenError(
				'Доступ запрещен. Возможно удаление только собственного твита'
			);
		})
		.catch((err) => {
			errorHandler(err, next, {
...
module.exports.likeTwit = (req, res, next) => {
	Twit.findByIdAndUpdate(req.params.twitId, { $addToSet: { likes: req.user._id } }, { new: true })
		.orFail()
		.then((twit) => res.send(twit))
		.catch((err) => {
			errorHandler(err, next, {
...
module.exports.dislikeTwit = (req, res, next) => {
	Twit.findByIdAndUpdate(req.params.twitId, { $pull: { likes: req.user._id } }, { new: true })
		.orFail()
		.then((twit) => res.send(twit))
		.catch((err) => {
			errorHandler(err, next, {
...
module.exports.getTwitComments = (req, res, next) => {
	...
	Twit.findById(twitId)
		.orFail()
		.then((twit) => res.send(twit.comments.length ? twit.comments : 'Комментарии не найдены'))
		.catch((err) => {
			errorHandler(err, next, {
				CastErrorMessage: 'Переданы некорректные данные',
				DocumentNotFoundErrorMessage: 'Твит с указанным id не найден'
...
module.exports.createTwitComment = (req, res, next) => {
	...
	Comment.create({ text, name, owner: req.user._id, parentTwit: twitId })
		.then((comment) => {
			...
			console.log(comment);
			Twit.findByIdAndUpdate(twitId, { $push: { comments: comment } }, { new: true })
				.orFail()
...
module.exports.editTwitComment = (req, res, next) => {
	...
	Comment.findById(commentId)
		...
		.then((comment) => {
			if (
				comment.parentTwit.toString() === twitId.toString() &&
				comment.owner.equals(req.user._id)
			) {
				Twit.findByIdAndUpdate(twitId, { $pull: { comments: comment } }, { new: true })
					.orFail()
					.catch((err) => {
						errorHandler(err, next, {
							CastErrorMessage: 'Переданы некорректные данные',
			...
			throw new ForbiddenError(
				'Доступ запрещен. Возможно редактирование только собственного комментария'
			);
		})
		.catch((err) => {
			errorHandler(err, next, {
...
module.exports.deleteTwitComment = (req, res, next) => {
	...
	Comment.findByIdAndRemove(commentId)
		...
		.then((comment) => {
			...
			throw new ForbiddenError(
				'Доступ запрещен. Возможно удаление только собственного комментария'
			);
		})
		.catch((err) => {
			errorHandler(err, next, {
